...
function writeUpdatedPOMFile(pomFolder, pom) {
    ...
    var pomFilePath = pomFolder + constants.SLASH + constants.POM_FILE_NAME;
    var pomFilePathOrig = pomFolder + constants.SLASH + constants.POM_FILE_NAME + constants.CONTENT_ORIG_EXT;
    var pomFilePathOld = pomFolder + constants.SLASH + constants.POM_FILE_NAME + constants.CONTENT_OLD_EXT;
    ...
    // Rename the file to 'origin' if there is none otherwise rename to 'old' and if 'old' already exists delete it first
    if(!fs.existsSync(pomFilePathOrig)) {
        console.log('Create Original POM File backup');
        fs.copyFileSync(pomFilePath, pomFilePathOrig);
    }
    if(fs.existsSync(pomFilePathOld)) {
        console.log('Delete existing Old File');
        fs.unlinkSync(pomFilePathOld);
    }
...
function pomFilter(file) {
    console.log('POM Filter File: ' + file.name);
    return file.name === constants.POM_FILE_NAME;
}
